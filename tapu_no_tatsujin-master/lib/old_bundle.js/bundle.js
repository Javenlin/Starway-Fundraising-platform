...
const startMessages = () => {
  let chooseLevel = new createjs.Text("選擇一個難度", "40px 微軟正黑體", "#000000");
  let beginPlay = new createjs.Text("來開始遊戲", "40px 微軟正黑體", "#000000");
  chooseLevel.x = 150;
  chooseLevel.y = 250;
  beginPlay.x = 40;
  beginPlay.y = 325;
...
const TaikoNew = (x=175, y= 525) => {
  let tapRed = new createjs.Bitmap("./assets/images/taiko_new.png");
  x=200; 
  tapRed.x = x;
  tapRed.y = y;
  tapRed.scaleX = tapRed.scaleY = 0.5;
...
const showScore = (score) => {
  let gameOver = new createjs.Text("遊戲結束!!", "45px 微軟正黑體", "white");
    gameOver.x = 75;
    gameOver.y = 200;
  let yourScore = new createjs.Text("您的分數是:", "45px 微軟正黑體", "white");
    yourScore.x = 65;
    yourScore.y = 255;
  let finalScore = new createjs.Text(score, "50px 微軟正黑體", "white");
    finalScore.x = 140;
    finalScore.y = 310;
    return (
...
class Game {
  ...
  tapNew() {
    if (this.pause === false) {
      Object(__WEBPACK_IMPORTED_MODULE_0__sounds__["b" /* Don */])().play();
      let tapGoldN = Object(__WEBPACK_IMPORTED_MODULE_1__tap__["c" /* TaikoGold */])(200);
  ...
  createRedTaps(speed=100){
    ...
    createNewTaps(speed=100){
      const that = this;
      let newTap = Object(__WEBPACK_IMPORTED_MODULE_1__tap__["n" /* TaikoRed */])(175, 0);
